...
The default `auto` mode checks the terminal environment, not the installed font file. It uses Nerd Font icons in recognized terminals and interactive UTF-8 TTYs. If icons appear as boxes or incorrect symbols, open `/open-tui` and choose one of these modes under **Appearance**:
...
- `auto`: use Nerd Font icons in recognized terminals and interactive UTF-8 TTYs; use ASCII when the output is not an interactive UTF-8 terminal
